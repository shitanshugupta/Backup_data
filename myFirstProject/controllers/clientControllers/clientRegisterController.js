...
const registerClient = async (req, res, next) => {
  ...
  if (
    ...
    const check=await client.findOne({where:{email:res2email}});
    ...
    let putData= await client.create(data);
    await client.update({status:resstatus},{where:{email:res2email}});
    res.json({
